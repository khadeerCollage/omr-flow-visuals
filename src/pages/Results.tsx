...
const Results = () => {
  ...
  const [selectedSheet, setSelectedSheet] = useState<SheetResult | null>(null);
  const [isReviewModalOpen, setIsReviewModalOpen] = useState(false);
  const [sortConfig, setSortConfig] = useState<{ key: keyof SheetResult; direction: 'asc' | 'desc' } | null>(null);
  const [isExporting, setIsExporting] = useState(false);

  useEffect(() => {
    // Simulate loading results
    const mockResults: SheetResult[] = [
      {
        id: "sheet-001",
        studentId: "ST001",
      ...
      },
      {
        ...
        percentage: 78,
        status: "Flagged",
        reviewNotes: "Ambiguous markings in questions 15-17"
      },
      {
        id: "sheet-004",
        studentId: "ST004",
  ...
  const handleSort = (key: keyof SheetResult) => {
  ...
  const handleReviewSheet = (sheet: SheetResult) => {
  ...
  const handleApproveSheet = () => {
    if (selectedSheet) {
      setResults(prev => 
        prev.map(result => 
          result.id === selectedSheet.id 
            ? { ...result, status: 'Completed' as const, reviewNotes: undefined }
            : result
        )
      );
      toast({
        title: "Sheet Approved",
        description: "The flagged sheet has been approved and marked as completed.",
      });
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p><span className="font-medium">Score:</span> {selectedSheet.score}/{selectedSheet.totalMarks}</p>
                </div>
                <div>
